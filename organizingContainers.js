...
function organizingContainers(container) {
   ...
   for(let i= 0; i < container.length; i++){
       ...
       for(let j=0; j < container[0].length;j++){
           acumFila = acumFila + container[i][j];
       }
       arrayFilas.push(acumFila);
   }

   for(let m=0; m < container[0].length;m++){
       acumColumna = 0;
       for(let n=0; n < container.length;n++){
           acumColumna = acumColumna + container[n][m];
       }
       arrayColumnas.push(acumColumna);
   }

   arrayFilas.sort(function(a,b) {return  b-a});
   arrayColumnas.sort(function(a,b) {return  b-a});

   let resultado = POSSIBLE;
   let contador=0;
   while(contador < arrayFilas.length && resultado == POSSIBLE){
       if(arrayFilas[contador] < arrayColumnas[contador]){
